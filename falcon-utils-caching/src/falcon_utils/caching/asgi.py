...
import falcon
import logging
from falcon_caching import AsyncCache as BaseCache
from falcon_caching.options import CacheEvictionStrategy, HttpMethods
from falcon_caching.async_middleware import Middleware, _DECORABLE_METHOD_NAME
...
class CacheMiddleware(Middleware):
    ...
    async def process_resource(self, req: falcon.Request, resp, resource, params):
        ...
        if not (await self.cache.has(req.context.cache.key)):
            return

        try:
            data = await self.cache.get(req.context.cache.key)
    ...
    async def process_response(self, req: falcon.Request, resp, resource, req_succeeded):
        cache_context: RequestCacheContext = req.context.cache
        ...
        if cache_context.state == CacheState.REQUIRES_CACHING:
            try:
                value = await self.serialize(req, resp, resource)
                timeout = cache_context.timeout
                if self.cache_config['CACHE_EVICTION_STRATEGY'] in [CacheEvictionStrategy.rest_based]:
                    # indefinite caching
                    timeout = 0

                await self.cache.set(cache_context.key, value, timeout=timeout)
